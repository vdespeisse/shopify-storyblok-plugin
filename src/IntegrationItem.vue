<template>
  <div v-if="item" class="integration-item" v-bind:class="{ 'integration-item--selected': selected }">
    <div class="integration-item__left">
      <img :src="imageUrl" class="integration-item__image">
    </div>
...
export default {
  ...
  computed: {
    ...
    imageUrl() {
      if (!this.item) return
      return this.item.images[0].transformedSrc
    }
  },
  methods: {
    truncate(string) {
