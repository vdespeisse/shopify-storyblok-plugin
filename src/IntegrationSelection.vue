  ...
  <div class="integration-selection">
    ...
    <div class="uk-grid integration-results" v-if="!loading">
      <div
        ...
        :key="product.id"
      >
        <a href="#" class="integration-product" @click.prevent="selectItem(product)">
          <IntegrationItem :item="product" :current="current"></IntegrationItem>
        </a>
      </div>
    </div>
...
export default {
  ...
  computed: {
    totalPages() {
      ...
      return this.products.length != 0
        ? Math.ceil(this.products.length / this.per_page)
        : 1;
    },
    requestOptions() {
  ...
  methods: {
    ...
    async load() {
      this.loading = true
      const products = await this.connector.getProducts()
